...
class TestVictoryDefeat(unittest.TestCase):
    ...
    def defeat_card(self, card_id, *, victory_first=True, rules=None,
                    is_scheme=True, attachment=False):
        scene = SceneLoader.NewScene('rhino', None, ['captain_marvel'], 115)
        scene.rules = [] if rules is None else rules
        commands = iter([
            'puzzle.ClearHand()',
            'puzzle.ChangeFormFor(0, "Hero")',
            'puzzle.PutIntoPlay("04047")',  # Skilled Investigator
            f'puzzle.PutIntoPlay("{card_id}")',
        ...
        def choose(prompt):
            ...
            if prompt.ability_type == 'ForcedInterrupt':
                suffix = 'Victory' if victory_first else 'When_Defeated'
                option = next((o for o in prompt.options if o['name'].endswith(suffix)), prompt.options[0])
        ...
        return game.world, target, operations, leave_play, response_states, chosen_orders

    def test_player_side_schemes_resolve_in_either_order(self):
        for rules in ([], ['v18_timing']):
            for card_id in ('41016', '43018', '48020'):
                for victory_first in (True, False):
                    with self.subTest(rules=rules, card=card_id, victory_first=victory_first):
                        world, target, operations, leaves, responses, choices = self.defeat_card(
                            card_id, victory_first=victory_first, rules=rules)
                        self.assertEqual(operations, [(victory_first, True, True),
                                                      (not victory_first, True, True)])
                        self.assertEqual(len(choices), 1)
                        self.assertTrue(choices[0].endswith('Victory' if victory_first else 'When_Defeated'))
    ...
    def test_encounter_scheme_keeps_optional_defeat_effect_before_responses(self):
        for victory_first in (True, False):
            with self.subTest(victory_first=victory_first):
                world, target, operations, leaves, responses, _ = self.defeat_card(
                    '16127', victory_first=victory_first)
                self.assertFalse(world.GetFirstPlayer().GetHero().IsExhaust())
                self.assertEqual(operations, [(victory_first, True, True),
                                              (not victory_first, True, True)])
                self.assertTrue(target.card.area.flags.is_victory_display)
    ...
    def test_victory_minion_leaves_play_once_for_victory_display(self):
        _, target, operations, leaves, _, _ = self.defeat_card('16183', is_scheme=False)
    ...
    def test_victory_attachment_still_moves_when_its_host_is_defeated(self):
        _, target, _, leaves, _, _ = self.defeat_card('01129', is_scheme=False, attachment=True)
    ...
    def test_explicit_legacy_timing_preserves_victory_destination(self):
        for card_id, is_scheme in (('16127', True), ('16183', False)):
            with self.subTest(card=card_id):
                _, target, _, leaves, _, choices = self.defeat_card(
